...
int main(int argc, char *argv[])
{
  ...
  //void * check = sbrk(0);
  //printf("Old sbrk location = %lu, check location = %lu\n", (unsigned long) end_segment_addr, (unsigned long) check);

  //data_segment_free_space = get_data_segment_free_space_size();
  //data_segment_free_space = thread_get_data_segment_free_space_size();
  ...
  //double elapsed_ns = calc_time(start_time, end_time);
  //printf("Execution Time = %f seconds\n", elapsed_ns / 1e9);
  //printf("Fragmentation (test data segment) = %f\n", (float)data_segment_free_space/(float) ((unsigned long)(end_segment_addr - start_segment_addr)));

  //printf("Fragmentation (sbrk calls) = %f\n", (float)data_segment_free_space/(float) data_segment_size);

  //printf("Test case data segment size = %lu, function data segment size = %lu\n", 
  //	 ((unsigned long)(end_segment_addr - start_segment_addr)), data_segment_size); 
  
  FILE * f = fopen("lock_report_output.csv", "a");
  if (f == NULL){
    fprintf(stderr,"File didn't open properly\n");
  }
  else{
    fprintf(f,"%f, %lu\n", elapsed_ns / 1e9, (unsigned long)(end_segment_addr - start_segment_addr));
      //, (float)data_segment_free_space/(float)data_segment_size);
    fclose(f);
  }
